...
### 🧪 Testing
- [x] **Component test harness untuk `MonitorAddItemModal.handleConfirm`** — selesai 2026-08-08, `components/monitor-add-item-modal.test.tsx` (14 test). Menutup keenam perilaku hardened: double-tap lock via `submitLock`, urutan `saved=true` sebelum `res.json()`, empat cabang status HTTP 400/401/404/409 plus jalur non-2xx generik, pencocokan `sort_order` tanpa fabrikasi id, tiga cabang hasil print + routing dua target, dan cabang `catch` berdasar `saved`. Tidak butuh msw — cukup stub `fetch`, sehingga `lib/print-dispatch.ts` dan `renderKitchenTicket` tetap jalan sungguhan. Tiap perilaku dibuktikan tidak palsu dengan mematikan baris yang dijaganya lalu memastikan test yang bersangkutan gagal; review independen mengulang pembuktian itu lewat 12 mutasi dan menemukan satu mutan yang selamat (dispatch minuman tidak pernah tereksekusi karena semua fixture ber-kategori makanan), yang lalu ditutup.
