...
- **State**: kolom `transactions.paid_at` (migrasi 0036, `timestamptz` NULL = belum bayar) — satu-satunya state, nempel per-transaksi. **Tanpa entitas meja**; pakai `customer_name` + `table_no` yg sudah ada. Partial index `idx_transactions_unpaid` predicate-nya masih bawa `is_takeaway = false`, jadi sejak 2026-08-08 sudah ga match query monitor lagi (query dilayani plain `created_at` index instead) — migrasi rebuild-nya sudah ditulis (`supabase/migrations/0038_monitor_unpaid_index_include_takeaway.sql`) tapi **belum diterapkan** ke database, jadi index yang aktif di DB sekarang masih predicate lama yang tidak berguna itu.
